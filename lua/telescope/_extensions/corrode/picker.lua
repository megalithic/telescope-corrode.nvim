...
return function(opts)
  ...
  local cmd = opts.fd_cmd or { "fd", "-t=f" }
  ...
  local find_command = finders.new_job(function(prompt)
    if not prompt or prompt == "" then
      local cmd = table.insert(opts.rg_cmd, { "--json", "--", "^", path })
        or { "rg", "-N", "--color=never", "--smart-case", "--json", "--", "^", path }
    ...
    cmd = table.insert(opts.rg_cmd, { "--json", "--", prompt, path })
      or { "rg", "-N", "--color=never", "--smart-case", "--json", "--", prompt, path }
    return cmd
